...
type ManagedPromise = Deferred<any, number> & { deadline: number };
...
type PromiseManager = {
    ...
    create: (timeout?: number) => { promiseId: number; promise: Promise<any> };
    /** Resolves (and removes) promise with given id by given value and returns whether it was present or not */
    resolve: (promiseId: number, value: any) => boolean;
 ...
 *
 * @returns Promise manager instance
 */
export const createPromiseManager = (options?: PromiseManagerOptions): PromiseManager => {
    const { timeout: defaultTimeout = 0, onTimeout } = options ?? {};
    const promises: ManagedPromise[] = [];
    ...
    const create = (timeout = defaultTimeout) => {
        const promiseId = ID++;
        const deferred = createDeferred<any, number>(promiseId);
    ...
    const resolve = (promiseId: number, value: any) => {
