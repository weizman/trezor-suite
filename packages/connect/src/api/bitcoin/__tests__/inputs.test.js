import { networks } from '@trezor/utxo-lib';
import { validateTrezorInputs } from '../inputs';
import * as fixtures from '../../tx/__fixtures__/inputs';

describe('core/methods/tx/inputs', () => {
    describe('validateTrezorInputs', () => {
        const coinInfo = { network: networks.bitcoin };
        fixtures.validateTrezorInputs.forEach(f => {
            it(f.description, () => {
                if (f.result) {
                    expect(validateTrezorInputs(f.params, coinInfo)).toMatchObject(f.result);
                } else {
                    expect(() => validateTrezorInputs(f.params, coinInfo)).toThrow();
                }
            });
        });
    });
});
