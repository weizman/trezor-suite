...
const pubNode2bjsNode = (node: Messages.HDNodeType, network?: Network) => {
...
const pubKey2bjsNode = (
    key: Messages.PublicKey | Messages.EthereumPublicKey,
    network?: Network,
) => {
...
export function xpubDerive<PK extends Messages.PublicKey | Messages.EthereumPublicKey>(
    xpub: PK,
    childXPub: PK,
    suffix: number,
    network?: Network,
...
export const xpubToHDNodeType = (xpub: string, network: Network): Messages.HDNodeType => {
...
export const convertMultisigPubKey = <T extends Messages.TxInputType | Messages.TxOutputType>(
    network: Network,
    utxo: T,
): T => {
