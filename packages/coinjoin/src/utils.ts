import { bufferutils, networks, address, script as bscript, payments } from '@trezor/utxo-lib';
...
export const updateAccountsUtxos = (
    accounts: ActiveRound['accounts'],
    newUtxos: RegisteredAccountUtxo[],
) =>
    Object.keys(accounts).reduce((result, key) => {
        const account = accounts[key];
        const accountUtxos = account.utxos.flatMap(
            utxo => newUtxos.find(u => u.outpoint === utxo.outpoint) || [],
        );
        result[key] = {
            ...account,
            utxos: accountUtxos,
