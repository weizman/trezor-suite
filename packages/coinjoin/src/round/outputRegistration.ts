...
import { sumCredentials, updateAccountsUtxos } from '../utils';
import {
    ActiveRound,
    ActiveRoundOptions,
    RegisteredAccount,
...
const registerOutput = async (
    ...
    options.log('registerOutput amount', amountCredentials, availableAmount);
    options.log('registerOutput vsize', vsizeCredentials, availableVsize);
...
export const outputRegistration = async (
    ...
    try {
        ...
        console.warn('AFTER REGISTRATION', usedAddresses);
        ...
        console.warn('--->ALL WRONG HERE', error);
